...
*/
	...
	elseif ($post_action=="Valider les modifications")
	{
		...
		$tmp_result_xml=set_entite_apps($post_appli,$id_host,"poste");
		...
		if ($tmp_result_xml["in"]>1)
			$result_xml.="s";
		$result_xml.=" et ".$tmp_result_xml["out"]." application";
	...
	echo "<tr bgcolor='white'>\n";
		echo "<th colspan='3'>";
		echo "<a href='poste_maintenance.php?tri2=".$tri2."&id_host=".$id_host."&ok=".$get_ok."&warning=".$get_warning."&error=".$get_error."' style='color:".$regular_lnk."'>Retour</a>";
		echo "</th>\n";
	echo "</tr>\n";
	echo "</table><br>\n";
	...
	$list_app=liste_applications();
	$list_app_poste=info_poste_appli_full($id_host);
	...
	foreach ($list_app as $lp)
	{
		...
		$bg=$dep_no_bg;
		$txt=$dep_no_txt;
		if (isset($list_app_poste[$lp["id_app"]]))
		{
			if ($list_app_poste[$lp["id_app"]]["poste"]==$id_host)
			{
				$lnk=$dep_entite_lnk;
				$bg=$dep_entite_bg;
				$txt=$dep_entite_txt;
			...
			if (isset($list_app_poste[$lp["id_app"]]["depends"]))
			{
				if ($temp2==0)
				{
					$lnk=$dep_depend_lnk;
				...
				foreach ($list_app_poste[$lp["id_app"]]["depends"] as $lp_dep)
				{
					if ($temp!="")
						$temp.= ", ";
					$temp.= "<a href='app_parcs.php?Appli=".$lp_dep."' style='color:".$lnk."'>".$list_app[hash('md5',$lp_dep)]["nom_app"]."</a>";
				}
			}
		}
		...
		echo "<input type='checkbox' id='appli[]' name='appli[]' value='".$lp["id_app"]."'";
		if ($temp2==1)
			echo " checked";
		echo " />";
		echo "</td>";
		echo "<td align='center'>";
		echo "<a href='app_parcs.php?Appli=".$lp["id_nom_app"]."' style='color:".$lnk."'>".$lp["nom_app"]."</a>";
		echo "</td>";
		echo "<td align='center'>";
		echo $lp["version_app"];
		echo "</td>";
