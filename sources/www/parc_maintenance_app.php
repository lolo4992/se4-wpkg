...
*/
	...
	$list_app_parc=info_parc_appli_full($get_parc);
	...
	foreach ($list_app as $lp)
	{
		...
		$bg=$dep_no_bg;
		$txt=$dep_no_txt;
		if (isset($list_app_parc[$lp["id_app"]]))
		{
			if ($list_app_parc[$lp["id_app"]]["parc"]==$get_parc)
			{
				$lnk=$dep_entite_lnk;
				$bg=$dep_entite_bg;
				$txt=$dep_entite_txt;
			...
			if (isset($list_app_parc[$lp["id_app"]]["depends"]))
			{
				if ($temp2==0)
				{
					$lnk=$dep_depend_lnk;
				...
				foreach ($list_app_parc[$lp["id_app"]]["depends"] as $lp_dep)
				{
					if ($temp!="")
						$temp.= ", ";
