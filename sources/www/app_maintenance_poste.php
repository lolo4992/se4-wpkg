...
//	include "ldap.inc.php";
	...
	elseif ($post_action=="Valider les modifications")
	{
		...
		$tmp_result_xml=set_appli_entites($post_host,"poste",$get_Appli);
		$result_xml="<center>Modification effectu&#233;e. Application ajout&#233;e sur ".$tmp_result_xml["in"]." poste";
		if ($tmp_result_xml["in"]>1)
			$result_xml.="s";
		$result_xml.=" et retir&#233;e sur ".$tmp_result_xml["out"]." poste";
	...
	$liste_poste_app=info_application_postes($get_Appli); // liste des parcs ou l'app est demandee avec info de dependance
	...
	foreach($liste_hosts as $host)
	{
		...
		if (!liste_poste_app)
			$test1=0;
		elseif (!array_key_exists($host["nom_poste"],$liste_poste_app))
			$test1=0;
		elseif (isset($liste_poste_app[$host["nom_poste"]]["poste"]))
			$test1=1;
		elseif (is_array($liste_poste_app[$host["nom_poste"]]["parc"]))
			$test1=2;
		elseif (is_array($liste_poste_app[$host["nom_poste"]]["required_by"]))
			$test1=3;
		else
			$test1=0;
		...
		echo "<input type='checkbox' name='host[]' id='host[]' value='".$host["id"]."'";
		if ($test1==1)
			echo " checked";
		echo " />";
		echo "</td>";
